...
def build_gui(start_cb=None, take_cb=None, stop_cb=None, history_cb=None):
    ...
    btn_start = ctk.CTkButton(button_frame_after, text="Начать игру", width=140, command=lambda: start_cb(name_entry.get()) if start_cb else None)
    btn_start.pack(side="left", padx=10)

    btn_history = ctk.CTkButton(button_frame_after, text="История", width=140, command=history_cb)
    btn_history.pack(side="left", padx=10)

    btn_take = ctk.CTkButton(button_frame, text="Взять карту", width=140, command=take_cb)
    btn_take.grid(row=0, column=0, padx=10)

    btn_stop = ctk.CTkButton(button_frame, text="Стоп", width=140, command=stop_cb)
